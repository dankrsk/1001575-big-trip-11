const getEventEditTemplate = (event) => {
  ...
  const getEventDetailsTemplate = (offers, info) => {
    ...
    if (offers.length || info) {
      return `<section class="event__details">
                ...
                ${getDestinationTemplate(eventInfo)}
              </section>`;
    }
  ...
  const startTime = getFormatedTime(event.time.start);
  const endTime = getFormatedTime(event.time.end);
  ...
  return (
    `<form class="event  event--edit" action="#" method="post">
				...
				<div class="event__type-wrapper">
					...
					<div class="event__type-list">
						<fieldset class="event__type-group">
							...
							${getEventItemsTemplate(eventType, `transfer`)}
						</fieldset>
						<fieldset class="event__type-group">
							<legend class="visually-hidden">Activity</legend>
							${getEventItemsTemplate(eventType, `activity`)}
						</fieldset>
					</div>
				</div>
				<div class="event__field-group  event__field-group--destination">
					<label class="event__label  event__type-output" for="event-destination-1">
          ${getEventWithAction(eventType)}
					</label>
      ...
      ${getEventDetailsTemplate(eventOffers, eventInfo)}
		</form>`
  );
};
